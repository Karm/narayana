 ...
 */
public class PostgresContainerAllocator extends Allocator {
    private static final Logger LOGGER = Logger.getLogger(PostgresContainerAllocator.class.getName());

    PostgresContainerAllocator() {
        // Use getInstance
    }

    public DB allocateDB(final int expiryMinutes) {
        final String projectBuildDirectory = System.getProperty("project.build.directory");
        if (StringUtils.isEmpty(projectBuildDirectory)) {
            throw new IllegalArgumentException("project.build.directory must not be empty");
        }
        ...
        final String containerDockerDaemonApiUrl = System.getProperty("container.docker.daemon.api.url");
        if (StringUtils.isEmpty(containerDockerDaemonApiUrl)) {
            throw new IllegalArgumentException("container.docker.daemon.api.url must not be empty");
        }
        final String containerDatabaseImage = System.getProperty("container.database.image");
        if (StringUtils.isEmpty(containerDatabaseImage)) {
            throw new IllegalArgumentException("container.database.image must not be empty");
        }
        final String containerName = System.getProperty("container.name");
        if (StringUtils.isEmpty(containerName)) {
            throw new IllegalArgumentException("container.name must not be empty");
        }
        final String containerDatabaseUsername = System.getProperty("container.database.username");
        if (StringUtils.isEmpty(containerDatabaseUsername)) {
            throw new IllegalArgumentException("container.database.username must not be empty");
        }
        final String containerDatabasePassword = System.getProperty("container.database.password");
        if (StringUtils.isEmpty(containerDatabasePassword)) {
            throw new IllegalArgumentException("container.database.password must not be empty");
        }
        final String containerDatabaseName = System.getProperty("container.database.name");
        if (StringUtils.isEmpty(containerDatabaseName)) {
            throw new IllegalArgumentException("container.database.name must not be empty");
        }
        final String containerDatabaseBindHostIp = System.getProperty("container.database.bind.host.ip");
        if (StringUtils.isEmpty(containerDatabaseBindHostIp)) {
            throw new IllegalArgumentException("container.database.bind.host.ip must not be empty");
        }
        final String containerDatabaseBindHostPort = System.getProperty("container.database.bind.host.port");
        final int port;
        if (StringUtils.isEmpty(containerDatabaseBindHostPort)) {
            throw new IllegalArgumentException("container.database.bind.host.port must not be empty");
        } else {
            port = Integer.parseInt(containerDatabaseBindHostPort);
        ...
        final String containerTimeoutWaitingForTcp = System.getProperty("container.timeout.waiting.for.tcp");
        final long timeout;
        if (StringUtils.isEmpty(containerTimeoutWaitingForTcp)) {
            throw new IllegalArgumentException("container.timeout.waiting.for.tcp must not be empty");
        } else {
            timeout = Long.parseLong(containerTimeoutWaitingForTcp);
            if (timeout > 30 * 60 * 1000 || timeout < 500) {
                throw new IllegalArgumentException("container.timeout.waiting.for.tcp out of expected range [500, 30*60*1000]");
            }
        }
        final String containerDatabaseDriverArtifact = System.getProperty("container.database.driver.artifact");
        if (StringUtils.isEmpty(containerDatabaseDriverArtifact)) {
            throw new IllegalArgumentException("container.database.driver.artifact must not be empty");
        }
        final String containerDatabaseDriverClass = System.getProperty("container.database.driver.class");
        if (StringUtils.isEmpty(containerDatabaseDriverClass)) {
            throw new IllegalArgumentException("container.database.driver.class must not be empty");
        }
        final String containerDatabaseDatasourceClassXa = System.getProperty("container.database.datasource.class.xa");
        if (StringUtils.isEmpty(containerDatabaseDatasourceClassXa)) {
            throw new IllegalArgumentException("container.database.datasource.class.xa must not be empty");
        }
        ...
        try {
            ...
            ex.printStackTrace();
            LOGGER.severe(String.format("Wasn't able to find image %s, see: %s", containerDatabaseImage, ex.getMessage()));
            return null;
        }
        ...
        while (!completed.get() && (System.currentTimeMillis() - timestamp < timeout)) {
            try {
                ...
                LOGGER.fine(String.format("Waiting for container...: %s", e.getMessage()));
            }
        }

        if (!completed.get()) {
            fail("Image was not pulled or container was not started in time.");
        }
    ...
    public boolean deallocateDB(final DB db) {
        ...
        dockerClient.listContainersCmd().withShowAll(true).exec().stream()
            ...
            try (InputStream x = dockerClient.copyArchiveFromContainerCmd(c.getId(), "/tmp/db.log").exec()) {
                ...
                LOGGER.severe("Failed to read logs from the database container.");
                e.printStackTrace();
            }
            if (s.getRunning() || s.getRestarting()) {
                LOGGER.info("Killing container.");

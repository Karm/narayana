...
public class BaseITCase extends AbstractCase {
    ...
    public void before() {
        // Clean Object store
        assertNotNull("CATALINA_HOME must be set.", System.getenv("CATALINA_HOME"));
        final File objectStore = new File(System.getenv("CATALINA_HOME") + File.separator + "bin" + File.separator + "ObjectStore");
        try {
            FileUtils.deleteDirectory(objectStore);
        } catch (IOException | NullPointerException e) {
            e.printStackTrace();
            fail(String.format("Failed to delete contents of objectStore: %s", objectStore.getAbsolutePath()));
        }
        // Build JAX-RS client

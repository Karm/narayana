...
public class DBAllocator extends Allocator {
    ...
    public DB allocateDB(final int expiryMinutes) {
        ...
        if (StringUtils.isEmpty(System.getProperty("dballocator.driver.url.timeout.seconds"))) {
            throw new IllegalArgumentException("dballocator.driver.url.timeout.seconds must not be empty");
        }
        final long driverFetchTimeoutSeconds = Long.parseLong(System.getProperty("dballocator.driver.url.timeout.seconds"));
        if (driverFetchTimeoutSeconds < 0 || driverFetchTimeoutSeconds > 1800) {
            throw new IllegalArgumentException("driverFetchTimeoutSeconds out of range 0 - 1800");
        }

        // Fetching driver
        final String dballocatorDriverUrl = System.getProperty("dballocator.driver.url");
        if (StringUtils.isEmpty(dballocatorDriverUrl)) {
            throw new IllegalArgumentException("dballocator.driver.url must not be empty");
        }
        ...
        final String dballocatorHostPort = System.getProperty("dballocator.host.port");
        if (StringUtils.isEmpty(dballocatorHostPort)) {
            throw new IllegalArgumentException("dballocator.host.port must not be empty");
        }
        final String dballocatorExpression = System.getProperty("dballocator.expression");
        if (StringUtils.isEmpty(dballocatorExpression)) {
            throw new IllegalArgumentException("dballocator.expression must not be empty");
        }
        final String dballocatorRequestee = System.getProperty("dballocator.requestee");
        if (StringUtils.isEmpty(dballocatorRequestee)) {
            throw new IllegalArgumentException("dballocator.requestee must not be empty");
        }
        final URL allocate;
        try {
        ...
        try (final InputStream i = new FileInputStream(dbAllocatorPropertiesFile)) {
            ...
            LOGGER.severe("Properties file doesn't exist, we cannot continue, see: " + e.getMessage());
            return null;
        }
    ...
    public DB allocateDB() {
        if (StringUtils.isEmpty(System.getProperty("dballocator.expiry.minutes"))) {
            throw new IllegalArgumentException("dballocator.expiry.minutes must not be empty");
        }
        return allocateDB(Integer.parseInt(System.getProperty("dballocator.expiry.minutes")));
    }
    ...
    public boolean deallocateDB(final DB db) {
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.severe("Failed to construct deallocate DB URL, see: " + e.getMessage());
            return false;
        }
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.severe("Failed to deallocate DB, see: " + e.getMessage());
            return false;
        }
    ...
    public boolean reallocateDB(final int expiryMinutes, final DB db) {
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.severe("Failed to construct reallocate DB URL, see: " + e.getMessage());
            return false;
        }
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.severe("Failed to reallocate DB, see: " + e.getMessage());
            return false;
        }
    ...
    public boolean reallocateDB(final DB db) {
        ...
        return reallocateDB(Integer.parseInt(System.getProperty("dballocator.expiry.minutes")), db);
    }
    ...
    public boolean cleanDB(final DB db) {
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.severe("Failed to construct clean DB URL, see: " + e.getMessage());
            return false;
        }
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.severe("Failed to clean DB, see: " + e.getMessage());
            return false;
        }
